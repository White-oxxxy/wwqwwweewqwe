...
@admin_router.message(CommandStart(), IsAdmin(settings.ADMIN_IDS), StateFilter(default_state))
async def process_command_start(message: Message):
    await message.answer(AllLexicon.answer_start.value, reply_markup=admin_menu_kb)

@admin_router.message(Command(commands="help"), IsAdmin(settings.ADMIN_IDS), StateFilter(default_state))
async def process_command_help(message: Message):
    await message.answer(AllLexicon.answer_help.value, reply_markup=admin_menu_kb)

@admin_router.message(IsAdmin(settings.ADMIN_IDS), F.text == AllLexicon.button_menu.value)
...
@admin_router.message(IsAdmin(settings.ADMIN_IDS), F.text == AdminLexicon.button_add.value)
